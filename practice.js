...
// 3、在传入的数中（脚本可选的数）找出最大的连续的五位数并返回，貌似题意很明确了~
...
function ipToInt2(ip) {
  return parseInt(ip.replace(/(\d)*\D+|(\d)*$/g, function (a) { return ('00000000' + (+a).toString(2)).slice(-8) }), 2)
}
...
var priceValue = maxAndmin('price', [{ "price": "6800" }, { "price": "7800" }, { "price": "8800" }, { "price": "9800" }, { "price": "11800" }, { "price": "13800" }]); //调用方式
console.log(priceValue.min, priceValue.max);


var [biggest, b] = Object.values({ a: 1, b: 3, c: 12, d: 8 }).sort((a, b) => b - a);
...
function concatAdd3(n) {
  ...
  fn.toString = fn.valueOf = function () { return n }
